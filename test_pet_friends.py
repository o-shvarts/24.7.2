...
def test_add_new_pet_without_name(name= "", animal_type="интеллектуал", age=7, pet_photo='images/images.jpeg'):
...
def test_get_api_key_for_valid_user_without_password(email= valid_email, password= ''):
    """Negative:   Проверка аторизации существующего пользователя с незаполненным полем 'password' """

    status, result = pf.get_api_key(email, password)

    assert 400 <= status < 500 , "Ожидаемый статус: 4xx, полученный статус: {}".format(status)
    assert 'key' not in result, "Авторизация не удалась"
